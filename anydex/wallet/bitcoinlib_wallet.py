...
class BitcoinlibWallet(Wallet):
    ...
    def __init__(self, wallet_dir, network, testnet):
        ...
        self.network = 'testnet' if self.TESTNET else network
    ...
    def get_name(self):
        return self.NETWORK_INFO[self.network][0]

    def get_identifier(self):
        return self.NETWORK_INFO[self.network][1]
    ...
    def get_balance(self):
        if self.created:
            ...
            return succeed({
                ...
                "pending": 0,
                "currency": self.NETWORK_INFO[self.network][1],
                "precision": self.precision()
            })

        return succeed({"available": 0, "pending": 0, "currency": self.NETWORK_INFO[self.network][1], "precision": self.precision()})
    ...
    def monitor_transaction(self, txid):
        ...
        monitor_task = self.register_task(f"{self.NETWORK_INFO[self.network][0]}_poll_{txid}", monitor, interval=5)
